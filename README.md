...
* 코틀린의 named arguemtns 를 잘 쓰는 것이 좋지 않을까?
  * 코드리뷰를 받으면서 named arguments 에 대해서 지적을 받았는데, 문득 드는 생각은 동일 데이터 타입에 대해서 순서의 보장을 해줄 수 있는 코틀린만의 장점이라고 생각해서 쓰는게 맞다고 판단한다.
  * IDE 에서 물론 힌트를 주지만, 힌트는 힌트일뿐 언제나 휴먼에러가 발생할 수 있을 것이고, 따로 해당 부분은 컴파일에서 에러가 나는 것이 아닌 런타임 환경에서 에러가 날 소지가 있기 때문이다.
```
